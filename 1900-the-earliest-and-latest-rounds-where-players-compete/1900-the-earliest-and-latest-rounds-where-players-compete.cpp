...
public:
    vector<int> earliestAndLatest(int n, int firstPlayer, int secondPlayer) {
        ...
        //base case when the first player and second player starts fighting in the intial round 
        if(left==n-right+1){
            return {1,1} ; 
        }
        // if the diff of position is negative we need to shift the places as it symmetrical it make no diff .
        if(left > n-right+1){
            int temp = n-left+1 ;
            left = n-right+1;
            right = temp ; 
